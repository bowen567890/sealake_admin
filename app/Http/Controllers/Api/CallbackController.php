...
class CallbackController extends Controller
{
    ...
    public function callback(Request $request)
    {
        ...
        //订单类型1余额提币2购买算力3签到触发4开通商家5购买积分6开通普通节点7开通超级节点
        if ($order->type==1){
            $this->withdraw($in);
            ...
            $this->openNormalNode($in);
        } else if ($order->type==7) {
            $this->openSuperNode($in);
        }
...
//                                                 $MyRedis->del_lock($signKey);
    ...
    //开通商家
    private function openMerchant($in)
    {
        $ordernum = $in['remarks'];
        
        $lockKey = 'callback:openMerchant:'.$ordernum;
        $MyRedis = new MyRedis();
//                                         $MyRedis->del_lock($lockKey);
        $ret = $MyRedis->setnx_lock($lockKey, 30);
        if(!$ret){
            Log::channel('open_merchant')->info('上锁失败', $in);
            die;
        }
        
        $order = MerchantOrderLog::query()->where(['ordernum'=>$ordernum, 'pay_status'=>0])->first();
        if (!$order) {
            Log::channel('open_merchant')->info('订单不存在', $in);
            $MyRedis->del_lock($lockKey);
            die;
        }
        ...
        {
            Log::channel('open_merchant')->info('币种不正确', $in);
            $MyRedis->del_lock($lockKey);
            die;
        }
        ...
        if (bccomp($order->dogbee, $amount, 2)>0) {
            if ($in['status']==3 && $order->pay_status==0) {
                Log::channel('open_merchant')->info('金额有误', $in);
                $order->pay_status = 2;
                $order->hash = $hash;
                $order->save();
                $this->setOrderStatus($ordernum, 2);
        ...
        if ($in['status']==3 && $order->pay_status==0)
        {
            ...
            $MerchantOrder->ordernum = $order->ordernum;
            $MerchantOrder->user_id = $order->user_id;
            $MerchantOrder->point = $order->point;
            $MerchantOrder->usdt_num = $order->usdt_num;
            $MerchantOrder->dogbee = $order->dogbee;
            $MerchantOrder->pay_type = $order->pay_type;
            $MerchantOrder->hash = $hash;
            $MerchantOrder->finish_time = $datetime;
            $MerchantOrder->save();
            
            User::query()->where('id', $order->user_id)->update(['is_merchant'=>1]);
            
            $userModel = new User();
            //分类1系统增加2系统扣除3开通商家4购买积分5转出扣除6转入获得7兑换算力
            $userModel->handleUser('point', $order->user_id, $order->point, 1, ['cate'=>3, 'msg'=>'开通商家', 'ordernum'=>$order->ordernum]);
...
//                                                 $MyRedis->del_lock($lockKey);
    ...
    //领取订单
    private function feeOrder($in)
    {
        $ordernum = $in['remarks'];
        
        $lockKey = 'callback:feeOrder:'.$ordernum;
        $MyRedis = new MyRedis();
//                                 $MyRedis->del_lock($lockKey);
        $ret = $MyRedis->setnx_lock($lockKey, 30);
        if(!$ret){
            Log::channel('fee_order')->info('上锁失败', $in);
            die;
        }
        
        $order = FeeOrder::query()->where(['ordernum'=>$ordernum, 'pay_status'=>0])->first();
        if (!$order) {
            Log::channel('fee_order')->info('订单不存在', $in);
            $MyRedis->del_lock($lockKey);
            die;
        }
        ...
        {
            Log::channel('fee_order')->info('币种不正确', $in);
            $MyRedis->del_lock($lockKey);
            die;
        }
        ...
        //支付类型1USDT(链上),2USDT+MH(链上),3VV(链上),4BNB(链上)
        if (bccomp($order->num, $amount, 6)>0) {
            if ($in['status']==3 && $order->pay_status==0) {
                Log::channel('fee_order')->info('金额有误', $in);
                $order->pay_status = 2;
                $order->hash = $hash;
                $order->save();
                $this->setOrderStatus($ordernum, 2);
        ...
        //判断归集地址
        if (isset($in['customeUser']) && $in['customeUser'] && $order->collection_address_map)
        {
            $customeUser = json_decode($in['customeUser'], true);
            $collection_address_map = json_decode($order->collection_address_map, true);
            
            $customeUserNum = count($customeUser);
            $collectionAddressMapNum = count($collection_address_map);
            if ($customeUserNum!=$collectionAddressMapNum) {
                Log::channel('fee_order')->info('归集地址不正确', $in);
                $MyRedis->del_lock($lockKey);
                die;
            }
            
            foreach ($customeUser as $key=>$vv)
            {
                if (!isset($collection_address_map[$key])) {
                    Log::channel('fee_order')->info('归集地址不正确', $in);
                    $MyRedis->del_lock($lockKey);
                    die;
                }
                $vv = strtolower($vv);
                $address = strtolower($collection_address_map[$key]);
                if ($vv!=$address) {
                    Log::channel('fee_order')->info('归集地址不正确', $in);
                    $MyRedis->del_lock($lockKey);
                    die;
        ...
        $pows = pow(10,18);
        //判断充值金额
        if (isset($in['customeAmount']) && $in['customeAmount'] && $order->collection_amount_map)
        {
            $customeAmount = json_decode($in['customeAmount'], true);
            $collection_amount_map = json_decode($order->collection_amount_map, true);
            foreach ($customeAmount as $key=>$vv)
            {
                if (!isset($collection_amount_map[$key])) {
                    Log::channel('fee_order')->info('归集地址金额不正确', $in);
                    $MyRedis->del_lock($lockKey);
                    die;
                }
                
                if (bccomp($collection_amount_map[$key], '0', 6)>0)
                {
                    $balance = $vv;
                    $balance = bcdiv($vv, $pows, 6);    //钱包系统返回来要除以18位
                    if (bccomp($balance, $collection_amount_map[$key], 6)!=0) {
                        Log::channel('fee_order')->info('归集地址金额不正确', $in);
                        $MyRedis->del_lock($lockKey);
                        die;
                    }
        ...
        if ($in['status']==3 && $order->pay_status==0)
        {
            ...
            $redeemKey = 'feeOrder:'.$order->type.':'.$order->user_id;
//                         $MyRedis->del_lock($redeemKey);
            $retRes = $MyRedis->setnx_lock($redeemKey, 30);
            if(!$retRes){
                Log::channel('fee_order')->info('数据上锁失败', $in);
                die;
            }
           
            $user = User::query()
                ->where('id', $order->user_id)
                ->first(['id','vv','vv_static','vv_dynamic','vv_quantify_wait']);
            
            if ($order->type==1) {
                $balance = 'vv_quantify_wait';
            } else if ($order->type==2) {
                $balance = 'vv_static';
            } else {
                $balance = 'vv_dynamic';
            ...
            $receive_num = bccomp($user->$balance, $order->receive_num, 6)>=0 ? $order->receive_num : $user->$balance;
            
            $date = $datetime = date('Y-m-d H:i:s', $time);
            $order->receive_num = $receive_num;
            $order->pay_status = 1;
            $order->hash = $hash;
            $order->finish_time = $date;
            $order->save();
            
            if (bccomp($receive_num, '0', 6)>0) 
            {
                $up = $vvData = [];
                
                if ($order->type==1) 
                {
                    //分类1系统增加2系统扣除3空投增加4领取扣除
                    UserVvQuantifyWait::query()->insert(
                        [
                            'ordernum' => $ordernum,
                            'user_id' => $user->id,
                            'from_user_id' => 0,
                    ...
                    //分类1系统增加2系统扣除3余额提币4提币驳回5领取空投
                    UserVvQuantify::query()->insert(
                        [
                            'ordernum' => $ordernum,
                            'user_id' => $user->id,
                            'from_user_id' => 0,
                    ...
                    $up['vv_quantify_wait'] = DB::raw("`vv_quantify_wait`-{$receive_num}");
                    $up['vv_quantify'] = DB::raw("`vv_quantify`+{$receive_num}");
                    $up['vv_quantify_total'] = DB::raw("`vv_quantify_total`+{$receive_num}");
                } 
                else if ($order->type==2) 
                {
                    //分类1系统增加2系统扣除3静态奖励4领取扣除
                    UserVvStatic::query()->insert(
                        [
                            'ordernum' => $ordernum,
                            'user_id' => $user->id,
                            'from_user_id' => 0,
                    ...
                    //分类1系统增加2系统扣除3余额提币4提币驳回5静态领取6动态领取
                    $vvData[] = [
                        'ordernum' => $ordernum,
                        'user_id' => $user->id,
                        'from_user_id' => 0,
                    ...
                    $up['vv_static'] = DB::raw("`vv_static`-{$receive_num}");
                    $up['vv_static_total'] = DB::raw("`vv_static_total`+{$receive_num}");
                    $up['vv'] = DB::raw("`vv`+{$receive_num}");
                } 
                else 
                {
                    //分类1系统增加2系统扣除3动态奖励4领取扣除
                    UserVvDynamic::query()->insert(
                        [
                            'ordernum' => $ordernum,
                            'user_id' => $user->id,
                            'from_user_id' => 0,
                    ...
                    $up['vv_dynamic'] = DB::raw("`vv_dynamic`-{$receive_num}");
                    $up['vv_dynamic_total'] = DB::raw("`vv_dynamic_total`+{$receive_num}");
                    $up['vv'] = DB::raw("`vv`+{$receive_num}");
                }
    ...
    //提币手续费订单
    private function withdrawFeeOrder($in)
    {
        $ordernum = $in['remarks'];
        
        $lockKey = 'callback:withdrawFeeOrder:'.$ordernum;
        $MyRedis = new MyRedis();
//                                         $MyRedis->del_lock($lockKey);
        $ret = $MyRedis->setnx_lock($lockKey, 30);
        if(!$ret){
            Log::channel('withdraw_fee_order')->info('上锁失败', $in);
            die;
        }
        
        $order = WithdrawFeeOrder::query()->where(['ordernum'=>$ordernum, 'pay_status'=>0])->first();
        if (!$order) {
            Log::channel('withdraw_fee_order')->info('订单不存在', $in);
            $MyRedis->del_lock($lockKey);
            die;
        }
        ...
        if (!isset($in['coin_token']) || $in['coin_token']!='BNB')
        {
            Log::channel('withdraw_fee_order')->info('币种不正确', $in);
            $MyRedis->del_lock($lockKey);
            die;
        }
        ...
        if (bccomp($order->num, $amount, 6)>0) {
            if ($in['status']==3 && $order->pay_status==0) {
                Log::channel('withdraw_fee_order')->info('金额有误', $in);
                $order->pay_status = 2;
                $order->hash = $hash;
                $order->save();
                $this->setOrderStatus($ordernum, 2);
        ...
        if (isset($in['customeUser']) && $in['customeUser'] && $order->collection_address_map)
        {
            ...
            $collectionAddressMapNum = count($collection_address_map);
            if ($customeUserNum!=$collectionAddressMapNum) {
                Log::channel('withdraw_fee_order')->info('归集地址不正确', $in);
                $MyRedis->del_lock($lockKey);
                die;
            }
        ...
        if (isset($in['customeAmount']) && $in['customeAmount'] && $order->collection_amount_map)
        {
            ...
            {
                if (!isset($collection_amount_map[$key])) {
                    Log::channel('withdraw_fee_order')->info('归集地址金额不正确', $in);
                    $MyRedis->del_lock($lockKey);
                    die;
                }
        ...
        if ($in['status']==3 && $order->pay_status==0)
        {
            ...
            $withdraw = Withdraw::query()
                ->where('status', 0)
                ->where('fee_order_id', $order->id)
                ->where('fee_status', 0)
                ->first();
            if (!$withdraw){
                Log::channel('withdraw_fee_order')->info('提币订单不存在', $in);
                $MyRedis->del_lock($lockKey);
                exit();
            }
            
            $redeemKey = 'withdrawFeeOrder:'.$withdraw->coin_type.':'.$order->user_id;
//                                     $MyRedis->del_lock($redeemKey);
            $retRes = $MyRedis->setnx_lock($redeemKey, 30);
            if(!$retRes){
                Log::channel('withdraw_fee_order')->info('提币数据上锁失败', $in);
                die;
            }
            ...
            try
            {     
                if ($withdraw->coin_type==2) {
                    $coin_type = 'mh';
                    $contractAddress = MainCurrency::query()->where('id', 2)->value('contract_address');
                }
                ...
                {
                    $coin_type = 'vv_quantify';
                    $contractAddress = MainCurrency::query()->where('id', 3)->value('contract_address');
                }
                
                $user = User::query()->where('id', $withdraw->user_id)->first(['id','wallet','mh','vv','vv_quantify']);
                if (bccomp($withdraw->num, $user->$coin_type, 6)>0)
                {
                    $order->pay_status = 1;
                    $order->hash = $hash;
                    $order->finish_time = $date;
                    $order->flag = 2;   //余额标记0尚未支付1余额充足2余额不足
                    $order->save();
                    ...
                    Log::channel('withdraw_fee_order')->info('提币余额不足', $in);
                    exit();
                }
                ...
                $order->hash = $hash;
                $order->finish_time = $date;
                $order->flag = 1;   //余额标记0尚未支付1余额充足2余额不足
                $order->save();
                
                $userModel = new User();
                $userModel->handleUser($coin_type, $user->id, $withdraw->num, 2, ['cate'=>3, 'msg'=>'余额提币', 'ordernum'=>$withdraw->ordernum]);
                
                $OrderLog = new OrderLog();
                $OrderLog->ordernum = $withdraw->ordernum;
                $OrderLog->user_id = $withdraw->user_id;
                $OrderLog->type = 1;    //订单类型1提币2购买鱼竿3购买硬币4购买鱼竿卡牌5幸运祈福
                $OrderLog->save();
            ...
            catch (\Exception $e)
            {
                DB::rollBack();
                $MyRedis->del_lock($redeemKey);
                Log::channel('withdraw_fee_order')->info($e->getMessage().$e->getLine());
            }
            $this->setOrderStatus($ordernum, 1);
        }
        $MyRedis->del_lock($lockKey);
