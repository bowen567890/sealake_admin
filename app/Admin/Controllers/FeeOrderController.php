...
class FeeOrderController extends AdminController
{
    public $payTypeArr = [
        4=>'BNB',
    ];
    ...
    {
        return Grid::make(new FeeOrder(), function (Grid $grid) {
            $grid->column('id')->sortable();
            $grid->column('user_id');
            $grid->column('num');
            $grid->column('receive_num');
...
//             $grid->column('collection_address_map');
            ...
            $grid->filter(function (Grid\Filter $filter) {
                ...
                $filter->equal('type')->select($this->typeArr);
//                 $filter->equal('pay_type')->select($this->payTypeArr);
            });
        });
    }
    ...
    {
        return Show::make($id, new FeeOrder(), function (Show $show) {
            $show->field('id');
            $show->field('user_id');
            $show->field('num');
    ...
    {
        return Form::make(new FeeOrder(), function (Form $form) {
            $form->display('id');
            $form->text('user_id');
            $form->text('num');
